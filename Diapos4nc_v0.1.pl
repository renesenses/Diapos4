...
sub is_an_image_file {
    ...
    my ($sfile,$spath,$sext) = fileparse($filename, qr/\.[^.]*/);
	my $exifTool_object = new Image::ExifTool;
	my $info = $exifTool_object->ImageInfo($filename);
...
# ASSUMES FILE EXISTS AND HAS A VALID MIME TYPE
...
# ON WORK : NOT DELETE ARBO YEAR EVENT WHEN REMOVING 
sub remove_only {
	if ( -f $File::Find::name ) {
		unlink($File::Find::name); 	
	}
	elsif ( $File::Find::dir =~ /^(\/[^\/]+){6}[0-9]{4}/ ) { 
	}
	elsif ( $File::Find::dir =~ /^(\/[^\/]+){6}/ ) {
			rmdir($File::Find::dir); 	
	}
}
